...
lambdaz, temp_sed, lci, obs_sed, fobs, efobs = \
    ...
    missing = (tempfilt['fnu'][:,idx] <= -9999) | (tempfilt['efnu'][:,idx] < 0)
    fobs = tempfilt['fnu'][:,idx]/lci**2*flam_factor
    efobs = tempfilt['efnu'][:,idx]/lci**2*flam_factor
    fobs[missing]  = np.nan #-99
    efobs[missing] = np.nan #-99
...
def plot_SED(idx=20, writePNG=True, MAIN_OUTPUT_FILE = 'photz',
             OUTPUT_DIRECTORY = 'OUTPUT', CACHE_FILE = 'Same',
             lrange=[3000,25000], axes=None, individual_templates=False,
             fnu=False, show_pz=False, snlim=3, scale_flambda=1.e-17,
             setrc=False, show_rest=False):
    """
    ...
    if axes is None:
        ...
        fig.subplots_adjust(wspace=0.18, hspace=0.0,left=0.09,bottom=0.15,right=0.95,top=0.9)
...
#        axp = fig.add_subplot(122)
    ...
    ax.errorbar(lci[highsn]/xrest, fobs[highsn], yerr=efobs[highsn], ecolor='black',
               color='black',fmt='o',alpha=alph, markeredgecolor='black',
               markerfacecolor='None', markeredgewidth=1.5, ms=8, zorder=2,
               label='Observed Flux')
    #
    ax.errorbar(lci[~highsn]/xrest, fobs[~highsn], yerr=efobs[~highsn], ecolor='0.7',
               color='black',fmt='o',alpha=alph, markeredgecolor='0.7',
               markerfacecolor='None', markeredgewidth=1.5, ms=8, zorder=1)
...
#    ax.semilogx()
    ...
    ax.set_ylim(-0.05*max(obs_sed),2.0*max(fobs))
    ax.set_xlabel(r'$\lambda%s$ [$\AA$]' %(rest_label))
    ax.set_ylabel(r'Flux Density $\rm [erg \ cm^{-2} \ s^{-1} \ \AA^{-1}]$')
    ax.legend(loc='best')
    ...
    if axes is None:
        return ax, (lci, obs_sed) #fig #[ax, axp]



def plot_SED2(idx=20, writePNG=True, MAIN_OUTPUT_FILE = 'photz',
             OUTPUT_DIRECTORY = 'OUTPUT', CACHE_FILE = 'Same',
             lrange=[3000,25000], axes=None, individual_templates=False,
             fnu=False, show_pz=False, snlim=3, scale_flambda=1.e-17,
             setrc=False, show_rest=False):
...
def plotExampleSED(idx=20, writePNG=True, MAIN_OUTPUT_FILE = 'photz', OUTPUT_DIRECTORY = 'OUTPUT', CACHE_FILE = 'Same', lrange=[3000,8.e4], axes=None, individual_templates=False, fnu=False, show_pz=True, snlim=2, scale_flambda=1.e-17, setrc=True, show_rest=False):
    ...
    if individual_templates:
        ...
        ax.plot(lambdaz/xrest, temp_sed, linewidth=1.5, color='blue',alpha=alph*1.0, zorder=-3)
...
zPhot_zSpec(zoutfile="./OUTPUT/photz.zout', zmax=4)
    ...
    zout = catIO.Table(zoutFile)#catIO.Readfile(zoutFile)
